  ...
  return (
    <article className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      ...
      <header className="mb-8">
        ...
        <p className="text-sm sm:text-base text-bakery-muted mt-2 max-w-3xl leading-relaxed">
          {content.explanation}
        </p>
      </header>
      ...
      <div className="border-t border-bakery-border pt-12 space-y-12">
        ...
        <section className="bg-white rounded-2xl border border-bakery-border p-6 sm:p-8 shadow-sm">
          ...
          <p className="text-sm sm:text-base text-bakery-muted leading-relaxed">
            {content.howItWorks}
          </p>
        </section>
        ...
        <section className="bg-white rounded-2xl border border-bakery-border p-6 sm:p-8 shadow-sm">
          ...
          <ul className="space-y-2 mt-2">
            {content.assumptions.map((item, idx) => (
              <li key={idx} className="text-sm text-bakery-muted flex items-start gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-bakery-accent mt-2 shrink-0" />
                <span>{item}</span>
              </li>
            ))}
          </ul>
          ...
          <div className="space-y-4">
            {content.commonMistakes.map((item, idx) => (
              <div key={idx} className="border-l-4 border-amber-400 pl-4 py-1">
                <h3 className="text-sm font-bold text-bakery-dark">{item.mistake}</h3>
                <p className="text-xs sm:text-sm text-bakery-muted mt-1 leading-relaxed">
                  {item.correction}
                </p>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {content.sources.map((src, idx) => (
              <div key={idx} className="p-4 rounded-xl bg-stone-50 border border-stone-200">
                <span className="font-bold text-sm text-bakery-dark block">{src.name}</span>
                <span className="text-xs font-semibold text-bakery-accent block">{src.author}</span>
                <p className="text-xs text-bakery-subtle mt-1.5 leading-relaxed">{src.note}</p>
              </div>
            ))}
          </div>
        </section>
            ...
            <div className="space-y-4">
              {tool.faqs.map((faq, idx) => (
                <details key={idx} className="group rounded-xl border border-stone-200 p-4 open:bg-stone-50/50 transition-colors">
                  <summary className="font-bold text-sm text-bakery-dark cursor-pointer select-none">
                    {faq.question}
                  </summary>
                  <p className="mt-2 text-xs sm:text-sm text-bakery-muted leading-relaxed">
                    {faq.answer}
                  </p>
                </details>
              ))}
